...
function RegisterPage() {
  ...
  const schema = z.object({
    name: z.string().trim().min(2, t("register.error.shortName")).max(80),
    email: z.string().trim().email(t("login.error.invalidEmail")).max(255),
    password: z.string().min(6, t("login.error.minPwd")).max(128),
    phone: z.string().trim().min(8, t("register.error.shortPhone")).max(20),
  ...
  const ROLE_OPTIONS = [
    { value: "dealer", label: t("register.role.dealer") },
    { value: "technicien", label: t("register.role.tech") },
    { value: "enqueteur", label: t("register.role.invest") },
    { value: "user", label: t("register.role.user") },
  ];
        ...
        <div className="w-full max-w-lg">
          ...
          <Card className="border-border/60 shadow-elegant">
            <CardContent className="p-8">
              <h1 className="text-2xl font-bold text-foreground mb-1">{t("register.h1")}</h1>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative mt-1.5">
                    ...
                    <Input id="password" type="password" value={form.password} onChange={(e) => set("password", e.target.value)} className="pl-9" placeholder="6+ caractères" required />
                  </div>
                </div>

                <div>
                  <Label>{t("register.profile")}</Label>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <Select value={form.type_activite} onValueChange={(v) => set("type_activite", v)}>
                        <SelectTrigger className="mt-1.5"><SelectValue /></SelectTrigger>
                        <SelectContent>
                          <SelectItem value="revente">{t("register.act.resale")}</SelectItem>
                          <SelectItem value="reparation">{t("register.act.repair")}</SelectItem>
                          <SelectItem value="import">{t("register.act.import")}</SelectItem>
                          <SelectItem value="autre">{t("register.act.other")}</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                  </div>
                ...
                <Button type="submit" className="w-full gradient-primary text-primary-foreground shadow-elegant h-11" disabled={loading}>
                  {loading ? <Loader2 size={16} className="animate-spin" /> : t("register.submit")}
                </Button>
              </form>
